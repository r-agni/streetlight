`, `erasableSyntaxOnly`, `moduleResolution: "bundler"`, ES2022) | `C:\Users\agni_\Documents\gaia\worldview\tsconfig.app.json` | **verbatim** |
| `eslint.config.js` (ESLint 9 flat, typescript-eslint + `reactHooks.configs.flat.recommended` + `reactRefresh.configs.vite`) | `C:\Users\agni_\Documents\gaia\worldview\eslint.config.js` | **verbatim** — this exact file is duplicated in `capital`, `hearth`, `physicalAI` and `systemScale-cloud/server/dashboard`, so it demonstrably *is* the house config |
| Tailwind v4 `@theme` token mechanism + the `touch-action: none` map fix + the `pointer-events: none` vignette | `C:\Users\agni_\Documents\gaia\worldview\src\index.css` | **mechanism verbatim**, values replaced by §1.5 |
| `WsManager.ts` — singleton, ref-counted `subscribe/unsubscribe`, exponential backoff capped at 30 s, `onclose` nulled before a deliberate close, token in the first message **body** not the URL | `C:\Users\agni_\Documents\systemScale-cloud\server\dashboard\src\ws\WsManager.ts` | **verbatim**, one line changed: `if (typeof data !== 'string') return` becomes the ArrayBuffer decode path, and set `ws.binaryType = 'arraybuffer'` |
...
| FastAPI module layout: `bridge/server.py` assembly-only, `bridge/routes/*.py`, `set_publisher()` injected singleton, frozen-dataclass config with module-level singleton overridden by env at the entrypoint only | `C:\Users\agni_\Documents\HADES\bridge\`, `C:\Users\agni_\Documents\HADES\hades\config.py` | **structure verbatim** |
| **Drift-corrected tick loop** | `C:\Users\agni_\Documents\HADES-main\bridge\server.py` | **verbatim.** Take this one, **not** `HADES\bridge\routes\stream.py`, which regressed to a naive `await asyncio.sleep(delay_s)` — that drifts by the frame-generation time on every tick, which at 10 Hz with 50k agents is not negligible |
| `frame_at(t)` pure-function publisher | `C:\Users\agni_\Documents\HADES-main\bridge\sim_publisher.py` | **architecture verbatim.** It is what makes the timeline scrubbable, the replay deterministic and the tests trivial |
| `geo.py` — `haversine_m`, `cumulative_distances`, `bearing_rad`, `decode_polyline`; equirectangular for lat/lng↔metres, spherical for distance and heading | `C:\Users\agni_\Documents\HADES\hades\realworld\geo.py` | **verbatim.** `cumulative_distances` is exactly the arc-length parameterization for placing an agent at fraction *t* along an SF street segment |
| **Shortest-path yaw blend** `atan2(sin(b-a), cos(b-a))` + `smoothstep` | `C:\Users\agni_\Documents\HADES-main\hades\motion.py` | **math only — vectorize it.** Without the yaw fix every bus and pedestrian spins a full turn each time heading crosses 0/360°, which is immediately visible. The surrounding `route_pose` is an O(segments) Python loop per agent per tick: correct for 8 drones, fatal at 50,000. Reimplement as `np.searchsorted` over a precomputed cumulative-distance array |
| rAF interpolation between low-rate WS frames, with the **first-frame snap** and the **sub-metre dead-band** | `C:\Users\agni_\Documents\HADES\viz\realworld-tempVisualizer\app.js` ~1660–1705, `MOTION_SMOOTH_MS = 220` | **adapt.** The dead-band is the anti-jitter fix that is easy to omit and annoying to diagnose. Apply the same smoothstep once per frame over typed arrays, not per object |
| Procedural canvas texture generation (`grain()`, `sketchLine()`, draw-once-upload-once) | `C:\Users\agni_\Documents\worldGen\lib\textures.ts` | **adapt** into `scripts/build-sprites.mjs` and `src/lib/halftone.ts` |
| `mulberry32` seeded PRNG; `makeToonGradient()` with `NearestFilter` (NearestFilter is what makes it *flat*) | `C:\Users\agni_\Documents\worldGen\lib\city.ts` | **adapt** — the PRNG is already in §2.1 |
| Easing library (`ease_out_cubic`, `ease_out_quint`, `ease_out_back`, `clamp01`, `EASINGS` registry) | `C:\Users\agni_\Documents\dashboard-agents\social\shorts\motion.py` | **port to TS** as `src/lib/easing.ts` |
| Imperative high-count-entity lifecycle: collection in a `useRef`, create/destroy in one effect keyed on the map, refill in a second, rich `id` payload for picking | `C:\Users\agni_\Documents\gaia\worldview\src\components\layers\FlightPointLayer.tsx` | **shape only** — deck.gl replaces the Cesium calls |
| Hoist WebGL/context constants to **module scope** so the map is not recreated per render | `C:\Users\agni_\Documents\gaia\worldview\src\components\globe\GlobeViewer.tsx` | **the lesson.** The identical trap exists with `@vis.gl/react-maplibre` + `MapLibreOverlay` |
| Demo fallback: phase counter on `setInterval`, positions via `useMemo` lerp + `sin` jitter | `C:\Users\agni_\Documents\gaia\worldview\src\hooks\useDemoShips.ts` | **adapt** so the front end is demoable before the Python engine exists |
| Frozen-dataclass settings gate with an explicit enable flag and a cheap default model | `C:\Users\agni_\Documents\HADES\hades\realworld\openai_settings.py` | **verbatim**, retargeted at Anthropic |
| Cost pre-flight gate before every generation call | `C:\Users\agni_\Documents\dashboard-agents\managementAgent\src\management_agent\services\creative.py` | **adapt** — directly useful when persona generation could run away |
| Module-level `STYLE_BLOCK` constant prepended to every prompt | `C:\Users\agni_\Documents\videoGen\scripts\generate\generate_scenes.py` | **adapt** for persona generation: one shared constant pins style and schema across N personas |
| The rule *"there is no LLM call anywhere in the render path, and that stays true"* | `C:\Users\agni_\Documents\dashboard-agents\social\shorts\critique.py` docstring | **adopt verbatim** as the sim-tick invariant |
| CLAUDE.md skeleton (Overview / Repository Structure tree / Quick Commands / Env Vars table / Architecture Decisions / Coding Conventions / Data Flow diagram / External APIs table / Testing / Known Considerations) | `C:\Users\agni_\Documents\gaia\worldview\CLAUDE.md` | **skeleton only.** That file has drifted badly — it documents `FlightLayer.tsx`, `TrafficLayer.tsx`, `CCTVLayer.tsx`, `CCTVPanel.tsx`, `useTraffic.ts` and `data/airports.ts`, none of which still exist. Budget a pass to keep the new one honest |
...
- Deferred work goes in `C:\Users\agni_\Documents\todo.md` with a reason and an unblocking trigger — "nothing 'to do later' lives only in chat."

**One conflict to resolve before the first commit.** `C:\Users\agni_\Documents\systemScale\CLAUDE.md` states, in the user's own words: *"Never include any AI attribution in commit messages — no 'Co-Authored-By: Claude', 'Made-with: Cursor', or any similar trailer from any AI tool."* The current session instructions mandate exactly such a trailer. These are directly opposed; ask which governs this repo rather than guessing, because the wrong choice is visible in every commit.
